...
def convert_predictions(batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
                        ...
                        input_reader: BaseInputReader, no_overlapping: bool = True):
...
def _convert_pred_entities(entity_types: torch.tensor, entity_spans: torch.tensor,
    ...
    # Limit number of entities to two
    try:
        values, indices = pred_entity_scores.sort(descending=True)
        pred_entity_scores = values[0:2]
        indices = indices[0:2]
        pred_entity_spans = torch.index_select(pred_entity_spans, 0, indices)
        pred_entity_types = torch.index_select(pred_entity_types, 0, indices)
    except:
        # Easy way to do nothing in the case only one entity has been specified
        pass
        ...
        for relation in sample_pred_relations:
            try:
                ...
                converted_relations.append(converted_relation)
		            ...
		            pass
                # This is simply an easy way to ignore the removed entities cases.
