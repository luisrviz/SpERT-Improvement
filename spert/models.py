...
class SpERT(BertPreTrainedModel):
    ...
    def __init__(self, config: BertConfig, cls_token: int, relation_types: int, entity_types: int,
        ...
        self.entity_classifier = nn.Linear(config.hidden_size * 2 + size_embedding, 64)
        self.second_entiti_classifier = nn.Linear(64,3)
        self.tanh = nn.Tanh()
        self.size_embeddings = nn.Embedding(200, size_embedding)
        self.dropout = nn.Dropout(prop_drop)
        self.gru = nn.GRU(input_size=config.hidden_size, hidden_size=config.hidden_size//2, bidirectional=True, batch_first=True).to('cuda')
    ...
    def _classify_entities(self, encodings, h, entity_masks, size_embeddings):
        ...
        output, hidden = self.gru(entity_spans_pool[-1])#2[:, -1, :].unsqueeze(0))
        hidden = hidden.view(1, 2, entity_masks.shape[1], h.shape[-1]//2)
        hidden = hidden[-1]
        hidden_forward, hidden_backward = hidden[0], hidden[1]
        hidden = torch.cat((hidden_forward, hidden_backward), dim=1)
        entity_spans_pool=hidden.unsqueeze(0)
        ...
        entity_clf = self.second_entiti_classifier(entity_clf)
